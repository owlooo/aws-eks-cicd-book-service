...
팀에서 만든 도서 관리 서비스 BookShelf를 AWS EKS에 배포하는 프로젝트입니다.

저는 프런트엔드·백엔드의 Docker 이미지와 EKS 배포 설정을 맡았습니다. 개발·운영 환경을 나누고, 부하에 따라 Pod가 늘어나는 과정까지 다뤘습니다.
...
프로젝트 당시 Source → Build → Approval → Deploy 단계가 완료된 운영 파이프라인입니다.
...
기존 데모 주소는 2026.09.20 확인 당시 접속되지 않았습니다. 위 화면은 프로젝트 당시 배포와 부하 테스트 기록입니다.
